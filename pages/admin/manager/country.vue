<template>
    <div class="">
        ...
        <OtherVTable class="" v-show="!isLoading" :refresh-cb="refreshData" :list-title="listTitle"
            :list-data="[...listData]" :cpn="AdminManagerVRecordCountry" />
...
<script setup>
import AdminManagerVRecordCountry from '~~/components/admin/manager/vRecordCountry';
...
const getMore = async (page, limit) => {
    try {
        ...
        const result = await useCountry.search(key.value, page, limit);

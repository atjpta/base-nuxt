<template>
    <div class="alert shadow-lg glass">
        <font-awesome-icon class="text-error" :icon="['fas', 'circle-exclamation']" />
        <div>
            <h3 class="font-bold">Error!</h3>
            <div class="text-xs">
                {{ notification?.message || "You have error" }}
            </div>
        </div>
        <div @click="close()" class="radial-progress text-error cursor-pointer" :style="time" style="--size: 50px">
            See
        </div>
    </div>
</template>
